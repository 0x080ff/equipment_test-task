        ...
        <v-form v-model="valid" ref="form">
          <v-container>
            <v-row>
              ...
              <v-col cols="12">
                <v-textarea v-model="formData.note" label="Note" variant="outlined"></v-textarea>
              </v-col>
            </v-row>
          </v-container>
        </v-form>
...
import snMask from '@/utils/snMask';
...
export default {
  ...
  setup(props, context) {
    ...
    const mask = computed(() => snMask(formData.type.mask));
    const typeRules = [v => !!v || 'Type is required'];
    const snRules = [
      v => !!v || 'SN is required',
