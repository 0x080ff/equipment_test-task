<template>
  <v-dialog :model-value="true" persistent>
    <v-card width="1000">
      ...
      <v-card-text>
        <v-form v-model="valid" ref="form">
          <v-container>
            <v-row>
              <v-col cols="6">
                <v-text-field v-model="formData.name" :rules="nameRules" label="Name" variant="outlined" required></v-text-field>
              </v-col>

              <v-col cols="6">
                <v-text-field v-model="formData.mask" :rules="maskRules" label="Mask" variant="outlined" required></v-text-field>
              </v-col>
            </v-row>
          </v-container>
        </v-form>
      ...
      <v-card-actions>
        <v-btn text color="red" @click="remove" v-if="item.id"> Delete </v-btn>
        <v-spacer></v-spacer>
        <v-btn text @click="close"> Cancel </v-btn>
        <v-btn text color="primary" variant="contained-text" @click="send"> Save </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
export default {
  ...
  setup(props, context) {
    ...
    const nameRules = [v => !!v || 'Name is required', v => v.length <= 255 || 'Name must be less than 255 characters'];
    const maskRules = [
